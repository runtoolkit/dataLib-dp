...
execute unless score #runtoolkit.packs.datalib.config.tick.rate datalib.meta matches 1.. run function datalib:api/config/tick_cfg

# If tick.rate is not >= 1, skip the real tick work and run the no-op once instead
execute if score #runtoolkit.packs.datalib.config.tick.rate datalib.meta matches ..0 run function datalib:runtime/bootstrap/empty
execute if score #runtoolkit.packs.datalib.config.tick.rate datalib.meta matches ..0 run return 0

execute if score #runtoolkit.packs.datalib.config.tick.rate datalib.meta matches 1.. run function datalib:runtime/tick
