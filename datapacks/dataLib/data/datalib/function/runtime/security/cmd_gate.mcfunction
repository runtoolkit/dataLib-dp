# ─────────────────────────────────────────────────────────────────
# datalib:runtime/security/cmd_gate
...
execute unless data storage datalib:engine global{loaded:1b} run function datalib:runtime/fallback/not_loaded
...
execute if entity @s[type=player] run execute unless score @s dl.perm_level >= #sec_req dl.tmp run function datalib:runtime/security/cmd_perm_denied
...
execute store success storage datalib:engine security.validation_status int 1 run function datalib:runtime/security/pipeline/f_2a83h

# Verify Canary Integrity (Zero-tick memory protection)
execute unless data storage datalib:engine security{canary:"0xCAFEBABE"} run return run function datalib:runtime/security/pipeline/canary_trigger

# Enforce confirmation for unrecognized/dangerous states
execute if data storage datalib:engine security{validation_status:0} run return run function datalib:runtime/security/pipeline/require_confirm
