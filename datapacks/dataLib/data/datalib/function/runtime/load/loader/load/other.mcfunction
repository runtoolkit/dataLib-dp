...
data modify storage datalib:input func set value "datalib:runtime/lib/sync_tick"
data modify storage datalib:input interval set value 20
data modify storage datalib:input key set value "sync_tick"
function datalib:runtime/lib/schedule with storage datalib:input {}
...
function datalib:runtime/tick/init_channels

# Assign pid for any players already online at load time
# (on_player_join won't fire for them after a /reload)
execute as @a run function datalib:internal/player/init_online