# datalib:runtime/load/loader/resolve/dependencies
# Resolves external/internal dependencies dataLib needs before the main
# load pipeline (datalib:runtime/load/loader/load/all) runs: origin watermark, fork status,
...
# datalib:runtime/load/loader/core/internal/load/fork_warn, which is a warning tier too —
...
function datalib:runtime/_rt_origin
# Absence = file removed or pack is a modified fork.
# WARN only — load is not aborted, but admins are notified.
execute unless data storage datalib:engine global{rt_origin_verified:1b} run function datalib:runtime/load/loader/core/internal/load/fork_warn
