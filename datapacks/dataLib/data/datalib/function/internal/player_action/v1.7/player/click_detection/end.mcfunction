...
# destroy entities
execute as @e[type=minecraft:interaction,tag=player_action.click_entity,predicate=datalib:player_action/v1/equals_uuid] run tp @s ~ -300 ~
execute as @e[type=minecraft:interaction,tag=player_action.click_entity,predicate=datalib:player_action/v1/equals_uuid] run kill @s
