...
execute if score $rw_inside dl.tmp matches 1 run execute if data storage datalib:engine _rw_cur.on_enter run function datalib:internal/geo/region_watch/fire_enter with storage datalib:engine _rw_cur
execute if score $rw_inside dl.tmp matches 1 run execute if data storage datalib:engine _rw_cur.on_enter_cmd run function datalib:internal/geo/region_watch/fire_enter_cmd with storage datalib:engine _rw_cur
execute if score $rw_inside dl.tmp matches 1 run return 0

# --- on_leave: was inside, now outside ---
$tag @s remove rw.$(id)
execute if data storage datalib:engine _rw_cur.on_leave run function datalib:internal/geo/region_watch/fire_leave with storage datalib:engine _rw_cur
execute if data storage datalib:engine _rw_cur.on_leave_cmd run function datalib:internal/geo/region_watch/fire_leave_cmd with storage datalib:engine _rw_cur
