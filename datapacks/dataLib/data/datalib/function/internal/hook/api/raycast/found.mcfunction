...
execute as @s run function datalib:modules/uuid/api/from_entity
data modify storage datalib:hook placed.uuid set from storage datalib:input value

execute as @s run function datalib:api/player/get_name
...
execute as @s run function datalib:internal/hook/fire with storage datalib:engine _hook_fire_tmp
data remove storage datalib:engine _hook_fire_tmp

# Legacy event system support (if present)
execute if score #m_hook datalib.Flags matches 1.. run function datalib:api/events/fire {id:"hook.placed"}
