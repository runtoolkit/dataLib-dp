# datalib:debug/menu/log/add (internal)
...
# /function datalib:debug/menu/tools/log/enable). No tellraw, no chat spam:
# this only touches a block's NBT + power state, which is what makes the
# game write the message line into latest.log.
$execute if data storage datalib:engine security{debug_log:1b} run function datalib:debug/menu/testblock/pulse {message:"[$(level)] $(message)"}
