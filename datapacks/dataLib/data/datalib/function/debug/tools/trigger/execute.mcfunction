# datalib:debug/menu/tools/trigger — single-call dispatch layer (EC-style).
# Does not require EventCore; uses AME's own datalib:api/cmd/* functions.
#
# Usage:
# function datalib:debug/menu/tools/trigger {type:"<type>", data:{...}}
# function datalib:debug/menu/tools/trigger {type:"<type>", data:{...}, config:{silent:1}}
#
# config:{silent:1} → suppress debug tellraw.

function datalib:debug/menu/tools/utils/check_all
...
# Execute action
function datalib:debug/menu/tools/trigger/dispatch
...
execute if data storage datalib:engine tools_trigger.config{silent:0} run function datalib:runtime/tick/disabled
